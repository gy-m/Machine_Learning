...
__global__ void contour(byte *dev_sobel_h, byte *dev_sobel_v, int gray_size, byte *dev_contour_img, int pixels_per_thread)
{
    ...
    while(tid < gray_size)
    {
    	...
    	{
    		//g = (g_x^2 + g_y^2)^0.5
			dev_contour_img[tid + i] = (byte) sqrt(pow((double)dev_sobel_h[tid + i], 2.0) + pow((double)dev_sobel_v[tid + i], 2.0));
    	}
...
__global__ void it_conv(byte * dev_buffer, int buffer_size, int width, int * dev_op, byte *dev_res, int pixels_per_thread)
{
...
__global__ void rgb_img_to_gray( byte * dev_r_vec, byte * dev_g_vec, byte * dev_b_vec, byte * dev_gray_image, int gray_size, int pixels_per_thread)
{
	...
	while(tid < gray_size)
	{
		for(int i = 0; i < pixels_per_thread; i++)
		{
			...
			byte p_r = dev_r_vec[tid + i];
			byte p_g = dev_g_vec[tid + i];
			byte p_b = dev_b_vec[tid + i];

			//Formula according to: https://stackoverflow.com/questions/17615963/standard-rgb-to-grayscale-conversion
			dev_gray_image[tid + i] = 0.30 * p_r + 0.59 * p_g + 0.11 * p_b;
		}
