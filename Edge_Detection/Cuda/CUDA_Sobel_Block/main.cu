...
#define INTERMEDIATE_OUTPUT true
#define SOBEL_OP_SIZE 9
#define STRING_BUFFER_SIZE 1024
//amount of pixels to be used for thread
...
int main ( int argc, char** argv )
{
		...
		if(argc < 3)
		{
			printf("Usage: ./Debug/CUDA_Sobel <input_image> <pixels_per_thread> \n");
			return -2;
		}
		...
		int pixels_per_thread =  atoi(argv[2]);
	    ...
	    rgb_img_to_gray <<< width, height>>> (dev_r_vec, dev_g_vec, dev_b_vec, dev_gray_image, gray_size, pixels_per_thread);
		...
		it_conv <<< width, height>>> (dev_gray_image, gray_size, width, dev_sobel_h, dev_sobel_h_res, pixels_per_thread);
	    cudaDeviceSynchronize();
		...
		it_conv <<<width, height>>> (dev_gray_image, gray_size, width, dev_sobel_v, dev_sobel_v_res, pixels_per_thread);
	    cudaDeviceSynchronize();
		...
		contour <<< width, height>>> (dev_sobel_h_res, dev_sobel_v_res, gray_size, dev_countour_img, pixels_per_thread);
	    cudaDeviceSynchronize();
